...
def generate_(random, args):
    ...
    return [random.randint(0, 18000) for i in range(size)] 
...
def perform_fitness(cs1, cs2, cs3, cs4, cs5, cs6, cs7, cs8, cs9, cs10, cs11, cs12):
    ...
    cc4 = np.round(cs11)
    ct4 = np.round(cs12)
    fit = float((cd1 * 0.31 + cc1 * 0.31 + ct1 * 0.31 + cd2 * 0.38 + cc2 * 0.38 + ct2 * 0.38 + cd3 * 0.35 + cc3 * 0.35 + ct3 * 0.35 + cd4 * 0.285 + cc4 * 0.285 + ct4 * 0.285) / 12151.56)

    #restrições
    #máximo de 10000kg na dianteira
    h1 = np.maximum(0, float(((cd1 + cd2 + cd3 + cd4) - 10000) / (10000/10)))

    #máximo de 16000kg na central
    h2 = np.maximum(0, float(((cc1 + cc2 + cc3 + cc4) - 16000) / (16000/10)))

    #máximo de 8000kg na traseira
    h3 = np.maximum(0, float(((ct1 + ct2 + ct3 + ct4) - 8000) / (8000/10)))

    #máximo de 18000kg da carga 1
    h4 = np.maximum(0, float(((cd1 + cc1 + ct1) - (18000)) / (18000/10)))

    #máximo de 15000kg da carga 2
    h5 = np.maximum(0, float(((cd2 + cc2 + ct2) - (15000)) / (15000/10)))

    #máximo de 23000kg da carga 3
    h6 = np.maximum(0, float(((cd3 + cc3 + ct3) - (23000)) / (23000/10)))

    #máximo de 12000kg da carga 4
    h7 = np.maximum(0, float(((cd4 + cc4 + ct4) - (12000)) / (12000/10)))

    #máximo de 6800m³ na dianteira
    h8 = np.maximum(0, float(((cd1 * 0.48 + cd2 * 0.65 + cd3 * 0.58 + cd4 * 0.39) - 6800) / (6800/10)))

    #máximo de 8700m³ na central
    h9 = np.maximum(0, float(((cc1 * 0.48 + cc2 * 0.65 + cc3 * 0.58 + cc4 * 0.39) - 8700) / (8700/10)))

    #máximo de 5300m³ na traseira
    h10 = np.maximum(0, float(((ct1 * 0.48 + ct2 * 0.65 + ct3 * 0.58 + ct4 * 0.39) - 5300) / (5300/10)))
...
def solution_evaluation(CD1, CC1, CT1, CD2, CC2, CT2, CD3, CC3, CT3, CD4, CC4, CT4):
    ...
    print("")
    print("Lucro total: ", float((CD1*0.31)+(CC1*0.31)+(CT1*0.31)+(CD2*0.38)+(CC2*0.38)+(CT2*0.38)+(CD3*0.35)+(CC3*0.35)+(CT3*0.35)+(CD4*0.285)+(CC4*0.285)+(CT4*0.285)))
    print("Volume no compartimento dianteiro", float((CD1*0.48)+(CD2*0.65)+(CD3*0.58)+(CD4*0.39)))
    print("Volume no compartimento central",float((CC1*0.48)+(CC2*0.65)+(CC3*0.58)+(CC4*0.39)))
    print("Volume no compartimento traseiro",float((CT1*0.48)+(CT2*0.65)+(CT3*0.58)+(CT4*0.39)))
    print("Total volumétrico: ",float((CD1*0.48)+(CD2*0.65)+(CD3*0.58)+(CD4*0.39)+(CC1*0.48)+(CC2*0.65)+(CC3*0.58)+(CC4*0.39)+(CT1*0.48)+(CT2*0.65)+(CT3*0.58)+(CT4*0.39)))
    print("")
    print("Peso no compartimento dianteiro", float(CD1 + CD2 + CD3 + CD4))
    print("Peso no compartimento central", float(CC1 + CC2 + CC3 + CC4))
    print("Peso no compartimento traseiro", float(CT1 + CT2 + CT3 + CT4))
    print("Total do peso: ",float(CD1 + CD2 + CD3 + CD4 + CC1 + CC2 + CC3 + CC4 + CT1 + CT2 + CT3 + CT4))
    print("")
    ...
    print("Lucro carga 1: ", float((CD1*0.31)+(CC1*0.31)+(CT1*0.31)))
    print("")
    ...
    print("Lucro carga 2: ", float((CD2*0.38)+(CC2*0.38)+(CT2*0.38)))
    print("")
    ...
    print("Lucro carga 3: ", float((CD3*0.35)+(CC3*0.35)+(CT3*0.35)))
    print("")
    ...
    print("Lucro carga 4: ", float((CD4*0.285)+(CC4*0.285)+(CT4*0.285)))
